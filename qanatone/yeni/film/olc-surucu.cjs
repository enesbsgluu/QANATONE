...
const TARA = `(async (bozBolum) => {
  ...
  const acilis = new Set(anims().filter((a) => a.tl === 'DocumentTimeline').map((a) => a.ad + '|' + a.hedef + a.ps));
  if (bozBolum) {
  ...
  for (const b of bolumler) {
    ...
    for (const a of anims()) {
      ...
      if (!isFinite(a.iter)) { dongu.add(a.ad + '@' + a.hedef); continue; }
      /* kadraja girisle BASLAYAN sonlu animasyon: startTime bolume gelisten
         sonra (300 ms rootMargin payi) — bitmis olsa da sayilir (kisa
         belirisler 120 ms'de bitebilir; yalniz 'running'e bakmak kacirir) */
      if (a.bas === null || a.bas < t0 - 300) continue;
      const k = a.ad + '|' + a.hedef + a.ps;
      if (acilis.has(k)) continue;
      kirmizi.push({ bolum: sec(b), anim: a.ad, hedef: a.hedef + a.ps, sure: a.sure });
    }
  }
...
(async () => {
  ...
  fs.writeFileSync(CIKTI, JSON.stringify({ _: 'yeni/film/olc-surucu.cjs — surucu kapisi: bolum gorununce oynayan sonlu DocumentTimeline animasyonu = kirmizi; view/scroll timeline serbest; sonsuz dongu ayri (sus).', olcum: new Date().toISOString(), kirmizi_kontrol: { yakalandi }, hukum: gecti ? 'GECTI' : 'KALDI', sayfa: sonuc }, null, 1));
